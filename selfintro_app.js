/* ============================================================
   ...
   ------------------------------------------------------------
   共有リンクは「id（ランダムID）＋復号鍵（URLのフラグメント）」の
   みで構成される。回答本体は暗号化されたうえで GAS 経由でスプレッド
   シートに保存され、復号鍵はサーバーに送信されない（URLの # 以降は
   ブラウザからサーバーへ送信されないため）。
   このアプリは継続的に編集するプロフィールという性質上、id・鍵は
   端末に保存して使い続け、共有タブを開くたびに同じidの内容を
...
/* ============================================================
   下書き保存／復元
   ============================================================ */
let shareId        = null; // 共有リンクの id（端末に保存し使い続ける）
let shareKeyBase64  = null; // 共有リンクの復号鍵（Base64。端末に保存）
...
function loadDraft(){
  try{
    ...
    shareKeyBase64=data.shareKeyBase64||null;
...
/* ============================================================
   共有データの組み立て・GASへの同期（暗号化して保存）
   ============================================================ */
function buildSharePayload(){
  const profile=collectProfile();
  return {
    profile: { name: profile.name||"", age: profile.age||"" }, // 生年月日は共有しない
    cards: collectCards(),
    torisetsu: collectTorisetsu(),
    schedule: schedulePatterns,
    createdAt,
...
/* ------------------------------------------------------------
   共有リンクを最新内容に同期する
   ・id/鍵が未発行なら新規発行し、以後は同じid/鍵を使い続ける
     （＝一度共有したリンクは、その後の編集内容にも追従する）
   ------------------------------------------------------------ */
async function syncShare(){
  const profile   = await liff.getProfile();
  const ownerHash = await sha256Hex(profile.userId);

  if(!shareId) shareId = (crypto.randomUUID ? crypto.randomUUID() : fallbackUUID());

  let key;
  if(shareKeyBase64){
    key = await importOwnerKey(shareKeyBase64);
  } else {
    const generated = await generateShareKey();
    key = generated.key;
    shareKeyBase64 = generated.base64;
  }

  saveDraft(); // id・鍵を端末に保存

  const cipherText = await encryptJSON(buildSharePayload(), key);

  const resp = await fetch(GAS_ENDPOINT, {
    method: "POST",
    headers: { "Content-Type": "text/plain;charset=utf-8" }, // preflight回避のため text/plain を使用
    body: JSON.stringify({ action: "share", id: shareId, cipherText, ownerHash, schemaVersion: 1 }),
  });
  const result = await resp.json();
  if(!result.ok) throw new Error(result.reason || "share_failed");

  return `${getFormBaseURL()}?id=${shareId}#${shareKeyBase64}`;
}

/* 本人が再暗号化（更新）するときに使う、encrypt/decrypt両対応のimport */
async function importOwnerKey(base64){
  const raw = base64UrlToBuf(base64);
...
function showStateCard(title,text,isLoading=false){
  ...
  pv.innerHTML=`
    <div class="timeline-header state-card">
      ${isLoading?`
      ...
      `:""}
      <p class="timeline-header-title state-title">${escapeHTML(title)}</p>
...
async function handleSharedView(id){
  showStateCard("読み込み中…","内容を確認しています。少々お待ちください。",true);
  ...
  try{
    ...
    showStateCard("通信エラー","内容を取得できませんでした。通信環境を確認してもう一度お試しください。");
    return;
  }
  ...
  if(!result.ok){
    if(result.reason==="forbidden"){
      ...
      showStateCard("エラー","内容を取得できませんでした。時間をおいて再度お試しください。");
    }
    return;
  }
...
/* 共有タブ表示中のURL欄をGAS同期しつつ更新する */
async function syncShareTabUI(){
  ...
  try{
    const url = await syncShare();
    input.value = url;
  }catch(e){
    console.error("share sync error",e);
    input.value = "";
    alert("共有リンクの準備に失敗しました。通信環境を確認してもう一度お試しください。");
  }finally{
...
function bindEvents(){
  ...
  document.getElementById("copyUrlBtn").addEventListener("click",async()=>{
    ...
    if(!inp.value){ await syncShareTabUI(); }
    if(!inp.value) return;
  ...
  document.getElementById("lineShareBtn").addEventListener("click",async()=>{
    const lineBtn=document.getElementById("lineShareBtn");
    let url=document.getElementById("shareUrlInput").value;
    if(!url){
      await syncShareTabUI();
